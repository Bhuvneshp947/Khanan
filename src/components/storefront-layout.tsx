...
export function ServiceTicker() {
  return (
    <div className="service-ticker overflow-hidden border-y border-foreground/20 bg-foreground py-3 text-background">
      <div className="ticker-track flex w-max whitespace-nowrap text-[0.65rem] font-bold uppercase tracking-[0.18em]">
        {[0, 1].map((group) => (
          <div key={group} className="flex items-center">
            {tickerItems.map((item) => (
              <span key={`${group}-${item}`} className="flex items-center gap-8 px-7 md:px-8">
                {item}
    ...
    <>
      <header className="fixed inset-x-0 top-0 z-50 border-b border-foreground/15 bg-background/80 backdrop-blur-xl">
        <div className="section-shell flex h-16 items-center justify-between">
          ...
          <Link to="/" className="group flex items-center transition-transform duration-300 hover:-translate-y-0.5"><img src={logoWordmark} alt="KHANAN" className="h-9 w-auto max-w-[10rem] object-contain md:h-11" /></Link>
          <nav aria-label="Main navigation" className="hidden items-center gap-9 text-[0.68rem] font-bold uppercase tracking-[0.18em] md:flex">
            {links.map((link) => <Link key={link.label} to={link.to} {...(link.hash ? { hash: link.hash } : {})} activeProps={{ className: "opacity-50" }} className="transition-all duration-300 hover:-translate-y-0.5 hover:opacity-70">{link.label}</Link>)}
          </nav>
          {onCart ? (
            <Button variant="ghost" onClick={onCart} className="relative text-[0.65rem] font-bold uppercase tracking-[0.14em] transition-transform duration-300 hover:-translate-y-0.5">Bag {cartCount > 0 && <span>({cartCount})</span>}</Button>
          ) : <div className="w-16" />}
        </div>
      </header>
      {menuOpen && (
        <div className="fixed inset-0 z-[90] bg-background p-4">
          <div className="flex items-center justify-between"><Link to="/" onClick={() => setMenuOpen(false)}><img src={logoWordmark} alt="KHANAN" className="h-9 w-auto max-w-[10rem] object-contain" /></Link><Button variant="ghost" size="icon" aria-label="Close menu" onClick={() => setMenuOpen(false)}><X /></Button></div>
          <nav className="mt-20 flex flex-col">{links.map((link) => <Link key={link.label} to={link.to} {...(link.hash ? { hash: link.hash } : {})} onClick={() => setMenuOpen(false)} className="display-type border-t border-foreground/20 py-5 text-6xl last:border-b transition-transform duration-300 hover:translate-x-2">{link.label}</Link>)}</nav>
        </div>
      )}
    </>
  );
...
export function SiteFooter() {
  ...
  const benefits = [
    ...
    { icon: CreditCard, title: "Cash on Delivery", text: "Pay your delivery partner when it arrives." },
  ];
  return (
