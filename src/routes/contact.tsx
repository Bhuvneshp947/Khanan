...
function ContactPage() {
  ...
  const sendMessage = async (event: FormEvent) => {
    ...
    try {
      const response = await fetch("/api/orders", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ message: body, subject: "KHANAN customer message" }) });
      const result = await response.json() as { ok?: boolean; mailto?: string };
      if (!response.ok || !result.ok) throw new Error("Message failed");
