  ...
  return (
    <PageFrame>
      ...
      <main ref={scope}>
        ...
        <section className="overflow-hidden border-y border-foreground/20 py-24 md:py-36"><div className="ticker-track flex w-max whitespace-nowrap text-[clamp(7rem,20vw,18rem)] text-foreground/10" aria-hidden="true">{[0, 1].map((group) => <span key={group} className="display-type shrink-0">MORE THAN A STORE — MORE THAN A STORE — </span>)}</div><div className="section-shell relative -mt-6 grid gap-10 md:-mt-16 md:grid-cols-2"><h2 data-reveal className="display-type text-6xl md:text-9xl">We are building the system behind the store.</h2><div data-reveal className="md:pt-14"><p className="text-lg leading-8 text-muted-foreground">The SYNOVA website is the beginning of a larger platform. We are continuously improving the way products are presented, discovered, purchased, delivered, and supported.</p><p className="mt-8 text-sm font-bold uppercase tracking-[0.18em]">Built for today. Designed to scale.</p></div></div></section>
