...
function Index() {
  ...
  // Refresh AOS when filter/slide changes so new items animate
  useEffect(() => {
    AOS.refresh();
  }, [filter, slide, visible]);
  ...
  return (
    <PageFrame>
      ...
      <main ref={scope}>
        ...
        <section className="relative overflow-hidden pt-24 md:pt-28">
          <div className="section-shell grid items-center gap-10 pb-16 md:grid-cols-2 md:gap-14 md:pb-24">
            <div key={`copy-${slide}`} className="hero-copy-enter" data-aos="fade-right">
              ...
              <h1 className="display-type mt-7 text-[clamp(3rem,11vw,7rem)]">
                {active.title.map((line) => (
                  <span key={line} className="block overflow-hidden">
                    <span data-page-title className="block">{line}</span>
                  </span>
                ))}
              </h1>
              <p className="mt-7 max-w-md text-sm leading-7 text-muted-foreground" data-aos="fade-up" data-aos-delay="150">
                {active.text}
              </p>
              <div className="mt-9 flex flex-wrap gap-3" data-aos="fade-up" data-aos-delay="250">
                <Button variant="editorial" size="editorial" asChild>
                  <a href="#products">{active.cta} <ArrowRight /></a>
                </Button>
                <Button variant="editorialOutline" size="editorial" asChild>
              ...
              <div className="mt-10 flex gap-2">
                {heroSlides.map((item, index) => (
                  <button
                    ...
                    className={`h-[3px] w-12 transition-all duration-500 ${index === slide ? "bg-foreground" : "bg-foreground/25 hover:bg-foreground/60"}`}
                  />
                ))}
              </div>
            </div>
            <div key={`media-${slide}`} className="hero-media-enter relative overflow-hidden rounded-2xl border border-foreground/15" data-aos="fade-left" data-aos-delay="100">
          ...
          <div className="section-shell flex items-center gap-3 pb-10 text-[0.62rem] font-bold uppercase tracking-[0.22em] text-muted-foreground">
            <ArrowDown className="soft-pulse size-4" /> Scroll to explore
          </div>
        </section>
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {trustCards.map((card, i) => (
              <article
                ...
                className="trust-card group relative overflow-hidden rounded-2xl border border-foreground/15 bg-card p-6 transition-all duration-500 hover:-translate-y-2 hover:border-foreground/60 hover:bg-foreground hover:text-background"
              >
                <span className="text-[0.6rem] font-bold tracking-[0.2em] opacity-50">{card.number}</span>
                <h2 className="display-type mt-12 text-4xl">{card.title}</h2>
                <p className="mt-3 text-xs uppercase tracking-[0.16em] opacity-60">{card.text}</p>
                <div className="mt-8 h-px w-8 bg-current transition-all duration-500 group-hover:w-full" />
              </article>
            ))}
          </div>
        </section>
        ...
        <section id="products" className="section-shell py-20 md:py-28">
          <div className="flex flex-wrap items-end justify-between gap-6 border-b border-foreground/20 pb-6" data-aos="fade-up">
            <div>
            ...
            <div className="flex flex-wrap gap-2">
              {categories.map((category) => (
                <button
                  ...
                  className={`rounded-full border px-4 py-2 text-[0.62rem] font-bold uppercase tracking-[0.16em] transition-all duration-300 ${filter === category ? "border-foreground bg-foreground text-background" : "border-foreground/25 hover:border-foreground hover:-translate-y-0.5"}`}
                >
                  {category}
                </button>
              ))}
          ...
          <div className="mt-12 grid gap-x-4 gap-y-12 sm:grid-cols-2 lg:grid-cols-3">
            {visible.map((product, index) => (
              <article
                key={product.name}
                data-aos="zoom-in"
                data-aos-delay={(index % 3) * 100}
                className="group cursor-hover cursor-pointer"
              ...
              >
                <div className="relative aspect-[4/5] overflow-hidden rounded-xl bg-card">
                  ...
                  <Button
                    ...
                    className="absolute bottom-3 right-3 transition-all duration-500 md:translate-y-4 md:opacity-0 md:group-hover:translate-y-0 md:group-hover:opacity-100"
                  >
                    <Plus />
                  </Button>
                </div>
                ...
                <p className="mt-3 flex flex-wrap items-center gap-3 text-sm">
                  ...
                  {product.was && <span className="text-xs text-muted-foreground line-through">{product.was}</span>}
                  <span className="text-[0.55rem] font-bold uppercase tracking-[0.14em] text-muted-foreground">0{index + 1}</span>
                </p>
              </article>
            ))}
          </div>
        ...
        <section className="relative h-[80svh] min-h-[520px] overflow-hidden">
          ...
          <div className="section-shell relative flex h-full flex-col justify-between py-14">
            <p data-aos="fade-down" className="text-[0.62rem] font-bold uppercase tracking-[0.22em]">
              Campaign 001 / Power, disclosed
            </p>
            <div data-aos="fade-up">
              <h2 className="display-type max-w-4xl text-[clamp(3.5rem,12vw,11rem)]">Charge different.</h2>
        ...
        <section id="categories" className="section-shell py-20 md:py-28">
          <p data-aos="fade-up" className="text-[0.62rem] font-bold uppercase tracking-[0.22em] text-muted-foreground">
            Shop by Category
          </p>
          <h2 data-aos="fade-up" data-aos-delay="100" className="display-type mt-5 text-6xl md:text-8xl">
            Find your gear
          </h2>
          <div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {categoryCards.map((category, i) => (
              <button
                ...
                data-aos-delay={i * 100}
                onClick={() => {
                  setFilter(category.name as (typeof categories)[number]);
                  document.getElementById("products")?.scrollIntoView({ behavior: "smooth" });
                }}
                className="group relative aspect-[3/4] overflow-hidden rounded-xl border border-foreground/15 text-left"
              >
                ...
                <div className="absolute inset-x-5 bottom-5">
                  ...
                  <span className="mt-2 flex items-center gap-2 text-[0.6rem] font-bold uppercase tracking-[0.18em] text-muted-foreground transition-all duration-300 group-hover:gap-4 group-hover:text-foreground">
        ...
        <section className="overflow-hidden border-y border-foreground/20 py-20 md:py-28">
          <div className="ticker-track flex w-max whitespace-nowrap text-[clamp(6rem,20vw,18rem)] text-foreground/10" aria-hidden="true">
            {[0, 1].map((group) => (
              <span key={group} className="display-type shrink-0">
                Radical transparency — Radical transparency —{" "}
              </span>
            ))}
          </div>
          <div className="section-shell relative -mt-6 grid gap-10 md:-mt-14 md:grid-cols-2">
            <h2 data-aos="fade-right" className="display-type text-6xl md:text-8xl">
              The label is the promise.
            </h2>
            <div data-aos="fade-left" className="md:pt-10">
              <p className="text-lg leading-8 text-muted-foreground">
                Original or master copy — every listing states exactly what you receive, with clear specs and honest pricing.
              </p>
              <div className="mt-9 grid grid-cols-2 gap-5 border-t border-foreground/20 pt-6 text-[0.62rem] font-bold uppercase tracking-[0.16em]">
                <span>01 / Checked by hand</span>
                <span>02 / Honest grading</span>
                <span>03 / Nationwide delivery</span>
                <span>04 / Email order updates</span>
              </div>
            </div>
          </div>
        </section>
      ...
      <div className={`fixed inset-y-0 right-0 z-[80] w-full max-w-md border-l border-foreground/20 bg-background p-5 transition-transform duration-500 ${cartOpen ? "translate-x-0" : "translate-x-full"}`} aria-hidden={!cartOpen}>
        <div className="flex items-center justify-between border-b border-foreground/20 pb-5">
          <h2 className="text-xs font-bold uppercase tracking-[0.2em]">Your bag ({cartCount})</h2>
        ...
        <div className="flex h-[calc(100%-5rem)] flex-col justify-between py-8">
          {cartCount === 0 ? (
            <div>
              <p className="display-type text-5xl">Your bag is quiet.</p>
              <p className="mt-4 text-sm text-muted-foreground">Add something worth carrying.</p>
            </div>
          ) : (
            <div className="overflow-y-auto">
              {cart.map(({ product, quantity }) => (
                <div key={product.name} className="flex gap-3 border-b border-foreground/15 py-4 first:pt-0">
                  <img src={product.image} alt={product.name} className="h-20 w-16 rounded-md object-cover" />
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-3 flex items-center gap-3">
                      <Button variant="ghost" size="icon" aria-label={`Remove one ${product.name}`} onClick={() => changeQuantity(product.name, -1)}>
                        <Minus />
                      </Button>
                      <span className="text-sm">{quantity}</span>
                      <Button variant="ghost" size="icon" aria-label={`Add one ${product.name}`} onClick={() => changeQuantity(product.name, 1)}>
                        <Plus />
                      </Button>
                    </div>
                  </div>
          ...
          <div className="border-t border-foreground/20 pt-5">
            ...
            <Button variant="editorial" size="editorial" disabled={cartCount === 0} onClick={beginCheckout} className="w-full">
              Checkout <ArrowRight />
            </Button>
          </div>
        </div>
      ...
      {selectedProduct && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center overflow-y-auto bg-background/80 p-4 backdrop-blur-md" role="dialog" aria-modal="true" aria-label="Product details">
          <div className="relative grid w-full max-w-3xl overflow-hidden rounded-2xl border border-foreground/20 bg-card md:grid-cols-2" data-aos="zoom-in">
            <button type="button" aria-label="Close product details" onClick={() => setSelectedProduct(null)} className="absolute right-3 top-3 z-10 flex size-9 items-center justify-center rounded-full bg-background/80 transition-transform hover:rotate-90">
              <X className="size-4" />
            </button>
            <img src={selectedProduct.image} alt={selectedProduct.name} className="h-72 w-full object-cover md:h-full" />
            <div className="p-7 md:p-10">
              <p className="text-[0.6rem] font-bold uppercase tracking-[0.2em] text-muted-foreground">{selectedProduct.brand || selectedProduct.category}</p>
              <h2 className="display-type mt-4 text-5xl">{selectedProduct.name}</h2>
              <p className="mt-5 text-sm leading-7 text-muted-foreground">{selectedProduct.details}</p>
              <p className="mt-7 text-2xl font-bold">{selectedProduct.price}</p>
              {selectedProduct.was && <p className="mt-1 text-xs text-muted-foreground line-through">{selectedProduct.was}</p>}
              <div className="mt-8 grid gap-3">
                <Button variant="editorial" size="editorial" onClick={() => addToCart(selectedProduct)}>
                  Add to cart <Plus />
                </Button>
                <Button
      ...
      {checkoutOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center overflow-y-auto bg-background/85 p-4 backdrop-blur-md" role="dialog" aria-modal="true" aria-label="Checkout">
          <div className="relative w-full max-w-2xl rounded-2xl border border-foreground/20 bg-card p-6 md:p-9">
            <button type="button" aria-label="Close checkout" onClick={() => setCheckoutOpen(false)} className="absolute right-4 top-4 flex size-9 items-center justify-center rounded-full transition-transform hover:rotate-90">
              <X className="size-4" />
            </button>
            {orderDone ? (
              <div className="py-12 text-center">
                ...
                <h2 className="display-type mt-7 text-6xl">Order received.</h2>
                <p className="mx-auto mt-5 max-w-md text-sm leading-7 text-muted-foreground">Your order details were sent to our team by email. Our delivery partner will call you on {customer.phone}.</p>
                <Button
                  variant="editorial"
                  size="editorial"
                  className="mt-8"
              ...
              <form onSubmit={submitOrder}>
                <p className="text-[0.6rem] font-bold uppercase tracking-[0.2em] text-muted-foreground">Secure checkout</p>
                <h2 className="display-type mt-3 text-6xl">Complete order</h2>
                <div className="mt-7 flex gap-2 border-b border-foreground/15 pb-3">
                  <button type="button" className={`text-xs font-bold uppercase tracking-[0.14em] ${accountMode === "signup" ? "opacity-100" : "text-muted-foreground"}`} onClick={() => setAccountMode("signup")}>
                    Sign up
                  </button>
                  <span className="text-muted-foreground">/</span>
                  <button type="button" className={`text-xs font-bold uppercase tracking-[0.14em] ${accountMode === "signin" ? "opacity-100" : "text-muted-foreground"}`} onClick={() => setAccountMode("signin")}>
                    Sign in
                  </button>
                </div>
                <div className="mt-6 grid gap-4 sm:grid-cols-2">
                  <label className="text-xs font-bold uppercase tracking-[0.12em]">
                    Full name
                    <input required value={customer.name} onChange={(event) => setCustomer({ ...customer, name: event.target.value })} className="mt-2 w-full border-b border-foreground/25 bg-transparent px-0 py-3 text-sm font-normal normal-case tracking-normal outline-none focus:border-foreground" placeholder="Your name" />
                  </label>
                  <label className="text-xs font-bold uppercase tracking-[0.12em]">
                    Email
                    <input required type="email" value={customer.email} onChange={(event) => setCustomer({ ...customer, email: event.target.value })} className="mt-2 w-full border-b border-foreground/25 bg-transparent px-0 py-3 text-sm font-normal normal-case tracking-normal outline-none focus:border-foreground" placeholder="you@example.com" />
                  </label>
                  <label className="text-xs font-bold uppercase tracking-[0.12em]">
                    Delivery phone
                    <input required type="tel" value={customer.phone} onChange={(event) => setCustomer({ ...customer, phone: event.target.value })} className="mt-2 w-full border-b border-foreground/25 bg-transparent px-0 py-3 text-sm font-normal normal-case tracking-normal outline-none focus:border-foreground" placeholder="03XX XXXXXXX" />
                  </label>
                  <label className="text-xs font-bold uppercase tracking-[0.12em]">
                    City
                    <input required value={customer.city} onChange={(event) => setCustomer({ ...customer, city: event.target.value })} className="mt-2 w-full border-b border-foreground/25 bg-transparent px-0 py-3 text-sm font-normal normal-case tracking-normal outline-none focus:border-foreground" placeholder="Lahore" />
                  </label>
                  <label className="text-xs font-bold uppercase tracking-[0.12em] sm:col-span-2">
                    Complete address
                    <textarea required value={customer.address} onChange={(event) => setCustomer({ ...customer, address: event.target.value })} className="mt-2 min-h-20 w-full resize-none border-b border-foreground/25 bg-transparent px-0 py-3 text-sm font-normal normal-case tracking-normal outline-none focus:border-foreground" placeholder="House, street, area" />
                  </label>
                </div>
                <div className="mt-8">
                  <p className="text-xs font-bold uppercase tracking-[0.12em]">Order summary</p>
                  <div className="mt-3 divide-y divide-foreground/15 border-y border-foreground/15">
                    {cart.map(({ product, quantity }) => (
                      <div key={product.name} className="flex items-start justify-between gap-4 py-3 text-sm">
                        <span className="max-w-[70%] leading-5">
                          {product.name} <span className="text-muted-foreground">x{quantity}</span>
                        </span>
                        <span className="shrink-0 font-bold">Rs. {(product.amount * quantity).toLocaleString()}</span>
                      </div>
                    ))}
                    <div className="flex items-center justify-between py-3 text-xs uppercase tracking-[0.12em] text-muted-foreground">
                      <span>Delivery</span>
                      <span>Free</span>
                    </div>
                ...
                <div className="mt-8">
                  <p className="text-xs font-bold uppercase tracking-[0.12em]">Payment method</p>
                  <div className="mt-3 flex items-center gap-3 rounded-xl border border-foreground bg-foreground p-4 text-background">
                    <MapPin className="size-5" />
                    <span>
                      <strong className="block text-sm">Cash on delivery (COD)</strong>
                      <small className="opacity-70">Pay our delivery partner when your order arrives</small>
                    </span>
                  </div>
                </div>
                {orderError && (
                ...
                <div className="mt-8 flex items-center justify-between border-t border-foreground/20 pt-5">
                  <div>
                    <p className="text-xs text-muted-foreground">Total to pay</p>
                    <p className="text-xl font-bold">Rs. {cartTotal.toLocaleString()}</p>
                  </div>
                  <Button variant="editorial" size="editorial" type="submit" disabled={submittingOrder}>
                    {submittingOrder ? "Sending..." : "Order now"} <ArrowRight />
                  </Button>
                </div>
                <p className="mt-4 flex items-center gap-2 text-[0.6rem] uppercase tracking-[0.12em] text-muted-foreground">
                  <User className="size-3" /> Your address is saved securely on this device for faster checkout.
                </p>
              </form>
            )}
          </div>
